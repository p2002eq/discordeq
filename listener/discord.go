...
func onMessageEvent(s *discordgo.Session, m *discordgo.MessageCreate) {
	commandchannel = config.Discord.CommandChannelID
...
func commandParse(s *discordgo.Session, m *discordgo.MessageCreate) {
	...
	if len(m.Message.Content) < 1 {
		if _, err := disco.SendMessage(commandchannel, fmt.Sprintf("%s: !help for valid commands", m.Author.Username)); err != nil {
	...
	allowedCommands := []string{"unlock", "kick", "who", "lock", "worldshutdown", "reloadquests"}
	//WorldCommands := []string{"unlock", "who", "lock", "worldshutdown"}
	//ZoneCommands := []string{"reloadquests"}
	...
	if command == "help" {
		if _, err := disco.SendMessage(commandchannel, fmt.Sprintf("%s: !help: Available commands: %s", m.Author.Username, strings.Join(allowedCommands[:], ", "))); err != nil {
			fmt.Printf("[Discord] Failed to send discord help command: %s\n", err.Error())
			return
		}
		return
	}
	//log.Printf("[DEBUG 1] Command Parse: %d with message: %s", channelMessage.Guilddbid, channelMessage.Message)
	...
	for _, cmd := range allowedCommands {
		...
		if err := SendWorldCommand(m.Author.Username, command, parameters); err != nil {
			if _, derr := disco.SendMessage(commandchannel, fmt.Sprintf("%s: %s: %s", m.Author.Username, command, err.Error())); derr != nil {
	...
	if _, err := disco.SendMessage(commandchannel, fmt.Sprintf("%s: %s is an invalid command. Use !help to learn commands.", m.Author.Username, command)); err != nil {
		fmt.Printf("[Discord] Failed to send discord command message: %s\n", err.Error())
		return
	}
